...
test.beforeAll(async () => {
  ...
  fixture = { incidentKey: result.receipt.incidentKey, planKey: result.receipt.plan.key, portfolioKey: result.receipt.portfolioKey, receiptDigest: canonicalDigest(result.receipt), repositories: result.receipt.inputs.map((input) => input.repository) };
});
