...
describe("HydraDB native contract", () => {
  ...
  it("refuses cursor, duplicate, cardinality, metadata, and endpoint mutations", () => {
    ...
    const duplicated = validateTraversalResponse(bounds, { sources: 1, targets: 1 }, { ...complete, records: [{ path }, { path }] }).refusalReasons; expect(duplicated).toEqual(expect.arrayContaining(["DUPLICATE_PAIR_ROWS", "RESULT_BOUND_EXCEEDED"]));
    expect(validateTraversalResponse(bounds, { sources: 0, targets: 1 }, complete).refusalReasons).toContain("SOURCE_CARDINALITY_MISMATCH"); expect(validateTraversalResponse(bounds, { sources: 1, targets: 0 }, complete).refusalReasons).toContain("TARGET_CARDINALITY_MISMATCH");
    expect(validateTraversalResponse(bounds, { sources: 1, targets: 1 }, { records: [{ path }] }).refusalReasons).toEqual(expect.arrayContaining(["READ_EPOCH_MISSING", "BOOKMARK_MISSING"]));
